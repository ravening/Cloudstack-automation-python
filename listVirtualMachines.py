...
def handleArguments(argv):
    ...
    help = "Usage: ./" + os.path.basename(__file__) + ' [options]' + \
        ...
        '\n  --displayname <vm display name>\tDisplay name of the vm' + \
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hc:d:p:o:f:z:n:",
            [
                "config-profile=", "domainname=", "displayname=",
...
def printVirtualmachine(args):
    ...
    if vmdata is not None:
        for vm in vmdata:
            ...
            # if its less than 1GB
            if vm.memory < 1024:
                print "memory is %d" % (vm.memory)
                vmmemory = str(vm.memory) + " MB"
...
if displayName.strip() is not None and displayName.strip() is not '':
#    vmID = c.checkCloudStackName({'csname': displayName,
#                                 'csApiCall': 'listVirtualMachines',
#                                 'listAll': 'true'})
    vmdata = c.listVirtualmachines({'instancename': displayName })
    t = PrettyTable([
        "VM",
        "Storage",
        "Template",
    ...
    t.align["VM"] = "l"
    projectParam = 'false'
    printVirtualmachine({'vmdata': vmdata })
    print t
    #print vmdata
    exit(1)
...
for clusterid, clustername in clusters.items():
    ...
    for fromHostData in fromClusterHostsData:
        ...
        for vm in vmdata:
            ...
            networkName = ''
            for nic in vm.nic:
                if nic.ipaddress is not None and nic.networkname is not None:
                    ipaddress = ipaddress + nic.ipaddress
                    networkName = networkName + nic.networkname
